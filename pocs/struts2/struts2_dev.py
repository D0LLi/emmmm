...
def poc(url, **kwargs):
    ...
    try:
        for test in poc_goop:
            req_get = safe_requests.get(url + '?' + test, headers=headers, timeout=timeout, verify=True)
            req_post = requests.post(url, data=test, headers=headers, timeout=timeout, verify=True)
